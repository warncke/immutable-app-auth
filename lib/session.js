...
function createSession (config, req, res) {
    ...
    // pseudo role
    session.roles.anonymous = 'Anonymous'
...
function loadAccount (config, req, res) {
    ...
    .then(roles => {
        ...
        _.each(roles, role => {
            session.roles[changeCase.paramCase(role.roleName)] = role.roleName
        })
    })
...
function loadSession (config, req, res, sessionId) {
    ...
    .then(all => {
        ...
        if (sessionAccount) {
            ...
            req.session.roles.authenticated = 'Authenticated'
        ...
        // session not linked to account
            ...
            req.session.roles.anonymous = 'Anonymous'
        }
    })
}
